...
def generar_grafico_barra(x_value, y_value):
    ...
    plt.figure(figsize=(10,5))
    plt.bar(x_vals, y_promedios, width=0.1)
    plt.xlabel(x_value)
    plt.ylabel(y_value)
    plt.title(f"{y_value} vs {x_value}")

    plt.savefig(f"static/plots/{f'{y_value} vs {x_value} barra'}.png")
    plt.close()

    return  


def generar_grafico_scatter(x_value, y_value):
    ...
    plt.figure(figsize=(10,5))
    plt.scatter(x_vals, y_promedios, alpha=0.3)
    plt.xlabel(x_value)
    plt.ylabel(y_value)
    plt.title(f"{y_value} vs {x_value}")

    plt.savefig(f"static/plots/{f'{y_value} vs {x_value} scatter'}.png")
    plt.close()
...
def graficos():
    ...
    #graficos cambiados a barra

    generar_grafico_barra("stress_level", "sleep_duration_hours")

    generar_grafico_barra("sleep_quality_score", "sleep_duration_hours")

    #graficos cambiados a scatter

    generar_grafico_scatter("daily_screen_time_hours", "mental_fatigue_score")
    generar_grafico_scatter("daily_screen_time_hours", "sleep_quality_score")

    generar_grafico_scatter("stress_level", "sleep_quality_score")

    generar_grafico_scatter("mental_fatigue_score", "sleep_quality_score")

    generar_grafico_scatter("phone_usage_before_sleep_minutes", "sleep_quality_score")
